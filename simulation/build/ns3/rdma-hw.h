...
struct RdmaInterfaceMgr{		//RDMA Interface Manager : RDMA接口管理器
	...
	//RdmaInterfaceMgr中，dev代表对应的端口设备，qpGrp代表对应的流
...
public:
	...
	//初始化m_cong
	void Gimbal_latency_congestion_init();
	//初始化m_rate
	void rate_init();
	//根据测量的延迟信息，返回拥塞状态
	int	Gimbal_congestion_update(uint32_t latency_ticks);
	//收到流完成时间后，调整目标速率target_rate
	void Gimbal_complete(uint32_t payload_size,uint32_t latency_ticks);
