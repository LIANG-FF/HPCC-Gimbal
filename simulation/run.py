...
'''
默认开启了QCN和PFC，也可以自己关掉
链路打开和链路时延在生成网络拓扑时写进入了
流量模型：目前仅测试纯8kb流量模型
'''

#默认配置模板，与config.txt里内容一样	
config_template="""ENABLE_QCN 1				#	开启QCN算法
USE_DYNAMIC_PFC_THRESHOLD 1					# 	开启PFC的Dynamic模式

PACKET_PAYLOAD_SIZE 1000					#	

TOPOLOGY_FILE mix/{topo}.txt				#	拓扑文件
FLOW_FILE mix/{trace}.txt					#	流文件
TRACE_FILE mix/trace.txt
TRACE_OUTPUT_FILE mix/mix_{topo}_{trace}_{cc}{failure}.tr		#输出的记录文件的名称
FCT_OUTPUT_FILE mix/fct_{topo}_{trace}_{cc}{failure}.txt
PFC_OUTPUT_FILE mix/pfc_{topo}_{trace}_{cc}{failure}.txt

SIMULATOR_STOP_TIME 4.00					#Simulator Stop Time 模拟结束时间
...
if __name__ == "__main__":
	...
	#添加参数:	--cc:命令行可选变量，dest:在这里的变量名 default:默认值 help:提示信息 action:暂时忽略https://blog.csdn.net/qq_43799400/article/details/119034026
	...
	bfsz = 16 * bw / 50					#buffer size 这个是交换机的buffer大小 
	u_tgt=args.utgt/100.				#HPCC参数
	mi=args.mi							
	pint_log_base=args.pint_log_base	#HPCC-PINT参数
	pint_prob = args.pint_prob			#HPCC-PINT
	...
	#
		...
		ai = 5 * bw / 25
		hai = 50 * bw /25
		config = config_template.format(bw=bw, trace=trace, topo=topo, cc=args.cc, mode=11, t_alpha=1, t_dec=4, t_inc=300, g=0.00390625, ai=ai, hai=hai, dctcp_ai=1000, has_win=0, vwin=0, us=0, u_tgt=u_tgt, mi=mi, int_multi=1, pint_log_base=pint_log_base, pint_prob=pint_prob, ack_prio=1, link_down=args.down, failure=failure, kmax_map=kmax_map, kmin_map=kmin_map, pmax_map=pmax_map, buffer_size=bfsz, enable_tr=enable_tr)
	else:
		print "unknown cc:", args.cc
		sys.exit(1)
	#===========================================================================================================

	#生成配置文件(在mix文件夹中)，供third.cc读取
